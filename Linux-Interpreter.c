...
void initialize_shell() // Greeting shell during startup 
{ 
	clear(); 
...
int parsediff(char* str, char** diffcom,int* lendiff) 
{ 
	int i=0; 
	while(str!=NULL)
	{
		diffcom[i] = strsep(&str,"&"); 
		i++;
	} 
	*lendiff=i;
	if (diffcom[1] == NULL) 
...
{ 
	...
	while (1) 
	{
		...
		if (takeInput(inputString)) // take input 
			continue; 
		parsediff(inputString,diffcom,lendiff); 
		pthread_t thread_id[*lendiff];
		...
		{
			error=pthread_create(&thread_id[i], NULL,&fn,(void*)diffcom[i]);
			if(error!=0)
				printf("\nThread can not be created : [%s]",strerror(error));
		}
		for(int i=0;i<*lendiff;i++)
		{
